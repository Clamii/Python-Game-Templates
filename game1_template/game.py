...
def menu(exits):
    ...
    while True:
        ...
        player_choice_of_exit = input()
        
        # Normalise the input
        
        player_choice_of_exit = normalise_input(player_choice_of_exit)
        
        # Check if the input makes sense (is valid exit)
            # If so, return the player's choice
            
        if is_valid_exit(exits, player_choice_of_exit):
            return player_choice_of_exit
...
def move(exits, direction):
    ...
    """
    for key in rooms:
        if rooms[key]["name"] == exit_leads_to(exits, direction):
            return rooms[key]
